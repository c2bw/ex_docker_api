...
defmodule DockerEngineAPI.Api.Image do
  ...
  - `opts` (keyword): Optional parameters
    - `:platform` ([String.t]): JSON-encoded OCI platform to select the image variant whose attestations to return. If omitted, the daemon's default (host) platform is used.  Only one platform value is currently accepted; passing more than one returns an error. The parameter is declared as an array so the wire shape can accept multiple values in the future without an API version bump.  Example: `{\"os\": \"linux\", \"architecture\": \"amd64\"}`
    - `:type` ([String.t]): In-toto predicate type URI to filter returned statements. May be repeated to accept any of several predicate types. If omitted, all statements are returned.  Example: `type=https://slsa.dev/provenance/v0.2&type=https://spdx.dev/Document`
    - `:statement` (boolean()): Include the verbatim in-toto statement body in each returned entry. Defaults to false; when omitted or false, only the descriptor and predicate type are returned and statement blobs are not read.

  ### Returns

  - `{:ok, [%AttestationStatement{}, ...]}` on success
  - `{:error, Tesla.Env.t}` on failure
  """
  @spec image_attestations(Tesla.Env.client, String.t, keyword()) :: {:ok, [DockerEngineAPI.Model.AttestationStatement.t]} | {:ok, DockerEngineAPI.Model.ErrorResponse.t} | {:error, Tesla.Env.t}
  def image_attestations(connection, name, opts \\ []) do
    optional_params = %{
      :platform => :query,
      :type => :query,
      :statement => :query
    ...
    |> Connection.request(request)
    |> evaluate_response([
      {200, DockerEngineAPI.Model.AttestationStatement},
      {400, DockerEngineAPI.Model.ErrorResponse},
      {404, DockerEngineAPI.Model.ErrorResponse},
      {500, DockerEngineAPI.Model.ErrorResponse},
      {501, DockerEngineAPI.Model.ErrorResponse}
    ])
  end
  ...
  - `opts` (keyword): Optional parameters
    ...
    - `:platforms` ([String.t]): Select platform-specific content to delete. Multiple values are accepted. Each platform is a OCI platform encoded as a JSON string.
  ...
  - `opts` (keyword): Optional parameters
    ...
    - `:platform` ([String.t]): JSON encoded OCI platform(s) which will be used to select the platform-specific image(s) to be saved if the image is multi-platform. If not provided, the full multi-platform image will be saved.  Example: `{\"os\": \"linux\", \"architecture\": \"arm\", \"variant\": \"v5\"}`
  ...
  - `{:ok, [%ImageHistoryResponseItem{}, ...]}` on success
  - `{:error, Tesla.Env.t}` on failure
  """
  @spec image_history(Tesla.Env.client, String.t, keyword()) :: {:ok, DockerEngineAPI.Model.ErrorResponse.t} | {:ok, [DockerEngineAPI.Model.ImageHistoryResponseItem.t]} | {:error, Tesla.Env.t}
  def image_history(connection, name, opts \\ []) do
    ...
    |> Connection.request(request)
    |> evaluate_response([
      {200, DockerEngineAPI.Model.ImageHistoryResponseItem},
      {404, DockerEngineAPI.Model.ErrorResponse},
      {500, DockerEngineAPI.Model.ErrorResponse}
    ])
  end
  ...
  - `name` (String.t): Image name or id
  - `opts` (keyword): Optional parameters
    - `:manifests` (boolean()): Include Manifests in the image summary.  The `manifests` and `platform` options are mutually exclusive, and an error is produced if both are set.
  ...
  - `opts` (keyword): Optional parameters
    ...
    - `:identity` (boolean()): Include `Identity` in each manifest summary. Requires `manifests=1`.
  ...
  - `opts` (keyword): Optional parameters
    ...
    - `:platform` ([String.t]): JSON encoded OCI platform(s) which will be used to select the platform-specific image(s) to load if the image is multi-platform. If not provided, the full multi-platform image will be loaded.  Example: `{\"os\": \"linux\", \"architecture\": \"arm\", \"variant\": \"v5\"}`
