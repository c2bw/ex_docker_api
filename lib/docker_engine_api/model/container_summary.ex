# NOTE: This file is auto generated by OpenAPI Generator 7.23.0 (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule DockerEngineAPI.Model.ContainerSummary do
  @moduledoc """

  """

  @derive JSON.Encoder
  defstruct [
    :Id,
    :Names,
    :Image,
    :ImageID,
    :ImageManifestDescriptor,
    :Command,
    :Created,
    :Ports,
    :SizeRw,
    :SizeRootFs,
    :Labels,
    :State,
    :Status,
    :HostConfig,
    :NetworkSettings,
    :Mounts,
    :Health
  ]

  @type t :: %__MODULE__{
    :Id => String.t | nil,
    :Names => [String.t] | nil,
    :Image => String.t | nil,
    :ImageID => String.t | nil,
    :ImageManifestDescriptor => DockerEngineAPI.Model.OciDescriptor.t | nil,
    :Command => String.t | nil,
    :Created => integer() | nil,
    :Ports => [DockerEngineAPI.Model.PortSummary.t] | nil,
    :SizeRw => integer() | nil,
    :SizeRootFs => integer() | nil,
    :Labels => %{optional(String.t) => String.t} | nil,
    :State => String.t | nil,
    :Status => String.t | nil,
    :HostConfig => DockerEngineAPI.Model.ContainerSummaryHostConfig.t | nil,
    :NetworkSettings => DockerEngineAPI.Model.ContainerSummaryNetworkSettings.t | nil,
    :Mounts => [DockerEngineAPI.Model.MountPoint.t] | nil,
    :Health => DockerEngineAPI.Model.ContainerSummaryHealth.t | nil
  }

  alias DockerEngineAPI.Deserializer

  def decode(value) do
    value
     |> Deserializer.deserialize(:ImageManifestDescriptor, :struct, DockerEngineAPI.Model.OciDescriptor)
     |> Deserializer.deserialize(:Ports, :list, DockerEngineAPI.Model.PortSummary)
     |> Deserializer.deserialize(:HostConfig, :struct, DockerEngineAPI.Model.ContainerSummaryHostConfig)
     |> Deserializer.deserialize(:NetworkSettings, :struct, DockerEngineAPI.Model.ContainerSummaryNetworkSettings)
     |> Deserializer.deserialize(:Mounts, :list, DockerEngineAPI.Model.MountPoint)
     |> Deserializer.deserialize(:Health, :struct, DockerEngineAPI.Model.ContainerSummaryHealth)
  end
end
