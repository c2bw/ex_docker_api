# NOTE: This file is auto generated by OpenAPI Generator 7.23.0 (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule DockerEngineAPI.Model.NetworkInspect do
  @moduledoc """

  """

  @derive JSON.Encoder
  defstruct [
    :Name,
    :Id,
    :Created,
    :Scope,
    :Driver,
    :EnableIPv4,
    :EnableIPv6,
    :IPAM,
    :Internal,
    :Attachable,
    :Ingress,
    :ConfigFrom,
    :ConfigOnly,
    :Options,
    :Labels,
    :Peers,
    :Containers,
    :Services,
    :Status
  ]

  @type t :: %__MODULE__{
    :Name => String.t | nil,
    :Id => String.t | nil,
    :Created => String.t | nil,
    :Scope => String.t | nil,
    :Driver => String.t | nil,
    :EnableIPv4 => boolean() | nil,
    :EnableIPv6 => boolean() | nil,
    :IPAM => DockerEngineAPI.Model.Ipam.t | nil,
    :Internal => boolean() | nil,
    :Attachable => boolean() | nil,
    :Ingress => boolean() | nil,
    :ConfigFrom => DockerEngineAPI.Model.ConfigReference.t | nil,
    :ConfigOnly => boolean() | nil,
    :Options => %{optional(String.t) => String.t} | nil,
    :Labels => %{optional(String.t) => String.t} | nil,
    :Peers => [DockerEngineAPI.Model.PeerInfo.t] | nil,
    :Containers => %{optional(String.t) => DockerEngineAPI.Model.EndpointResource.t} | nil,
    :Services => %{optional(String.t) => map()} | nil,
    :Status => DockerEngineAPI.Model.NetworkStatus.t | nil
  }

  alias DockerEngineAPI.Deserializer

  def decode(value) do
    value
     |> Deserializer.deserialize(:IPAM, :struct, DockerEngineAPI.Model.Ipam)
     |> Deserializer.deserialize(:ConfigFrom, :struct, DockerEngineAPI.Model.ConfigReference)
     |> Deserializer.deserialize(:Peers, :list, DockerEngineAPI.Model.PeerInfo)
     |> Deserializer.deserialize(:Containers, :map, DockerEngineAPI.Model.EndpointResource)
     |> Deserializer.deserialize(:Status, :struct, DockerEngineAPI.Model.NetworkStatus)
  end
end
